...
fs::path internal::generate_unique_folder(const fs::path& working_directory)
{
     ...
     auto threadNumber(std::stoul(boost::lexical_cast<std::string>(boost::this_thread::get_id())));
...
void internal::ltrim(std::string &str)
{
     str.erase(str.begin(), std::find_if(str.begin(), str.end(),
					 [](int s) {return !(std::isspace(s) == 0); }));
}

void internal::rtrim(std::string &str)
{
     str.erase(std::find_if(str.rbegin(), str.rend(),
			    [](int s) {return !(std::isspace(s) == 0);}).base(),
	       str.end());
}
...
std::string internal::get_output_filename(const std::string& command_line)
{
     ...
     // First guess: output is the last token
     auto output = tokens.back();
